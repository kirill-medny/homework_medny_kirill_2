...
class TestCurrencyConversion(unittest.TestCase):
    ...
    @patch('src.external_api.requests.get')  # Мокаем requests.get
    def test_conversion_usd(self, mock_get):
        transaction = {
            "operationAmount": {
                "amount": 100,
                "currency": {"code": "USD"}
    ...
    @patch('src.external_api.requests.get')  # Мокаем requests.get
    def test_conversion_eur(self, mock_get):
        transaction = {
            "operationAmount": {
                "amount": 100,
                "currency": {"code": "EUR"}
    ...
    @patch('src.external_api.requests.get')  # Мокаем requests.get
    def test_conversion_no_amount(self):
        transaction = {
            "operationAmount": {
                "currency": {"code": "USD"}  # Нет amount
            }
        }
        result = currency_conversion(transaction)
        self.assertEqual(result, 0.0)  # Проверяем, что результат равен 0.0
    ...
    @patch('src.external_api.requests.get')  # Мокаем requests.get
    def test_conversion_api_error(self, mock_get):
        transaction = {
            "operationAmount": {
                "amount": 100,
                "currency": {"code": "USD"}
    ...
    @patch('src.external_api.requests.get')  # Мокаем requests.get
    def test_conversion_request_exception(self, mock_get):
        transaction = {
            "operationAmount": {
                "amount": 100,
                "currency": {"code": "USD"}
