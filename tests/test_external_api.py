...
class TestCurrencyConversion(unittest.TestCase):
    ...
    def test_conversion_rub(self) -> None:
    ...
    @patch("src.external_api.requests.get")  # Мокаем requests.get
    def test_conversion_usd(self, mock_get) -> None:
    ...
    @patch("src.external_api.requests.get")  # Мокаем requests.get
    def test_conversion_eur(self, mock_get) -> None:
    ...
    @patch("src.external_api.requests.get")  # Мокаем requests.get
    def test_conversion_invalid_currency(self) -> None:
    ...
    @patch("src.external_api.requests.get")  # Мокаем requests.get
    def test_conversion_no_amount(self) -> None:
        transaction = {"operationAmount": {"currency": {"code": "USD"}}}  # Нет amount
        result = currency_conversion(transaction)
        self.assertEqual(result, 0.0)  # Проверяем, что результат равен 0.0

    @patch("src.external_api.requests.get")  # Мокаем requests.get
    def test_conversion_api_error(self, mock_get) -> None:
        transaction = {"operationAmount": {"amount": 100, "currency": {"code": "USD"}}}
    ...
    @patch("src.external_api.requests.get")  # Мокаем requests.get
    def test_conversion_request_exception(self, mock_get) -> None:
        transaction = {"operationAmount": {"amount": 100, "currency": {"code": "USD"}}}
